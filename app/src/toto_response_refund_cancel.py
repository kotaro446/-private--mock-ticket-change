...
def resp_refund_cancel(req:dict, segments:list, mapping_num:int) -> Response:
    """
    TOTO払戻取消要求のレスポンスを返す。
    ...
    """
    # 必須項目チェック
    if validate_request_fields(req, toto_constant.VALIDATE_REQUEST_FIELDS_REFUND_CANCEL):
    ...
    if mapping_num == 3:
        return resp_refund_cancel_map_clearing(req, segments, resp_dict)

    # No.3_払戻・払戻取消応答_マッピング
    return resp_refund_cancel_map_refund(req, segments, resp_dict)
...
def resp_refund_cancel_map_refund(req:dict, segments:list, resp_dict:dict) -> Response:
    """
    No.3_払戻・払戻取消応答_マッピング
    TOTO払戻取消要求のレスポンスを返す。
    ...
    if req["X_saiso_kbn"] == "1":
        if get_X_saiso_kbn_for_refund_cancel_map_refund(segments[7]) == "0":
            return resp_refund_cancel_map_refund_case_resend(req, segments, resp_dict)

    # 処理結果の判定
    X_shori_kekka, httpstatus = get_X_shori_kekka_for_refund_cancel_map_refund(segments[4])
    if X_shori_kekka != "00":
        return generate_error_response(X_shori_kekka, httpstatus)

    return resp_refund_cancel_map_refund_case_success(req, segments, resp_dict)


def resp_refund_cancel_map_refund_case_success(req:dict, segments:list, resp_dict:dict) -> Response:
    """
    No.3_払戻・払戻取消応答_マッピング
    ...
    """
    # 応答結果が "00" の場合、設定する
    resp_dict["X_outou_kekka"] = get_X_outou_kekka_for_refund_cancel_map_refund(segments[5])
    if resp_dict["X_outou_kekka"] == "00":
        ...
        resp_dict["X_payback_amt"] = get_X_payback_amt_for_refund_cancel_map_refund(segments[6])
...
def resp_refund_cancel_map_refund_case_resend(req:dict, segments:list, resp_dict:dict) -> Response:
    """
    No.3_払戻・払戻取消応答_マッピング
...
def resp_refund_cancel_map_clearing(req:dict, segments:list, resp_dict:dict) -> Response:
    """
    ...
    """
    # 処理結果の判定
    X_shori_kekka, httpstatus = get_X_shori_kekka_for_refund_cancel_map_clearing(segments[1])
    if X_shori_kekka != "00":
        return generate_error_response(X_shori_kekka, httpstatus)
    
    # 応答結果が "00" の場合、設定する
    resp_dict["X_outou_kekka"] = get_X_outou_kekka_for_refund_cancel_map_clearing(segments[2])
    if resp_dict["X_outou_kekka"] == "00":
        resp_dict["X_shop_id"] = "30801"
        resp_dict["X_haraimodoshi"] = req["X_barcode"]
        resp_dict["X_payback_amt"] = get_X_payback_amt_for_refund_cancel_map_clearing(segments[3])
