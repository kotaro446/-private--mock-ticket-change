...
def resp_purchase(req:dict, segments:list, mapping_num:int) -> Response:
    """
    TOTO購入要求のレスポンスを返す。
    ...
    """
    # 必須項目チェック
    if validate_request_fields(req, toto_constant.VALIDATE_REQUEST_FIELDS_PURCHASE):
    ...
    if mapping_num == 1:
        return resp_purchase_map_vote(req, segments, resp_dict)
    
    # No.1_購入・購入取消応答_マッピングの場合
    return resp_purchase_map_purchase(req, segments, resp_dict)
...
def resp_purchase_map_purchase(req:dict, segments:list, resp_dict:dict) -> Response:
    """
    ...
    if req["X_saiso_kbn"] == "1":
        if get_X_saiso_kbn_status_for_purchase_map_purchase(segments[4]) == "0":
            return resp_purchase_map_purchase_case_resend(req, segments, resp_dict)

    # 処理結果の判定
    X_shori_kekka, httpstatus = get_X_shori_kekka_for_purchase_map_purchase(segments[0])
    if X_shori_kekka != "00":
        return generate_error_response(X_shori_kekka, httpstatus)

    # 正常レスポンス
    return resp_purchase_map_purchase_case_success(req, segments, resp_dict)


def resp_purchase_map_purchase_case_resend(req:dict, segments:list, resp_dict:dict) -> Response:
    """
    No.1_購入・購入取消応答_マッピング
    再送の場合のTOTO購入要求のレスポンスを返す。
    ...
    resp_dict["X_tc_dai"] = get_X_tc_dai_for_purchase_map_purchase(segments[2])
...
def resp_purchase_map_purchase_case_success(req:dict, segments:list, resp_dict:dict) -> Response:
    """
    No.1_購入・購入取消応答_マッピング
    ...
    """
    # 応答結果を取得する
    resp_dict["X_outou_kekka"] = get_X_outou_kekka_for_purchase_map_purchase(segments[1])
...
def resp_purchase_map_vote(req:dict, segments:list, resp_dict:dict) -> Response:
    """
